...
//根据行为实体id和文章列表id查询app行为表  ap_show_behavior
        ...
        List<Integer> integerList = Arrays.asList(articleIds);
        if (!apShowBehaviors.isEmpty()) {
            apShowBehaviors.forEach(item -> {
                Integer articleId = item.getArticleId();
                integerList.remove(articleId);
            });
        }
        //保存操作
        if (integerList != null) {
            System.out.println(articleIds);
            articleIds = new Integer[integerList.size()];
            integerList.toArray(articleIds);
            apShowBehaviorMapper.saveShowBehavior(idWorker.nextId(), articleIds, apBehaviorEntry.getId());
        }
