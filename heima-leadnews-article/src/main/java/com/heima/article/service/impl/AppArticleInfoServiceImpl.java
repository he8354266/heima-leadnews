...
public class AppArticleInfoServiceImpl implements AppArticleInfoService {
    ...
    public ResponseResult loadArticleBehavior(ArticleInfoDto dto) {
        ...
        ApCollection apCollection = apCollectionMapper.selectForEntryId(apBehaviorEntry.getId(), dto.getArticleId(), ApCollection.Type.ARTICLE.getCode());
        if (apCollection != null) {
            isCollection = true;
        }
        // 3，通过entryId和articleId去查看点赞表，看是否有数据，有数据就说明已经点赞，否则就是没有点赞
        ApLikesBehavior apLikesBehavior = apLikesBehaviorMapper.selectLastLike(apBehaviorEntry.getId(), dto.getArticleId(), ApCollection.Type.ARTICLE.getCode());
        ...
        if (user != null && apAuthor != null && apAuthor.getUserId() != null) {
            ApUserFollow apUserFollow = apUserFollowMapper.selectByFollowId(user.getId(), apAuthor.getUserId().intValue());
